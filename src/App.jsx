...
function App() {
  ...
  return (
    <>
      <Nav scrolled={scrolled} activeSection={activeSection} onNav={goTo}/>
      <Hero onRequestCollect={() => goTo('contato')} isMobile={isMobile}/>
      {tweaks.showStats && <StatStrip/>}
      <Steps variant={tweaks.stepsVariant}/>
      <Services/>
      <About/>
      <Compliance/>
      <CTA onRequest={() => window.scrollTo({ top: 0, behavior: 'smooth' })}/>
      <Footer onNav={goTo}/>
      <TweaksPanel tweaks={tweaks} setTweaks={setTweaksPersist} visible={tweaksOn}/>
    </>
  );
}
      ...
      <div className="resp-grid-2 resp-inner" style={{
        ...
        {items.map((it, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ fontFamily: 'Archivo', fontSize: 20, fontWeight: 700, color: 'white', letterSpacing: '-0.01em' }}>
              {it.v}
            </div>
          </div>
        ))}
